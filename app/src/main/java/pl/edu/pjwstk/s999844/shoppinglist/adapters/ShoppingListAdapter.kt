...
class ShoppingListAdapter(private val changeAmountCallback: BiConsumer<RequiredItem, Int?>) : RecyclerView.Adapter<ShoppingListAdapter.ViewHolder>() {
	companion object {
		private const val CROSSOUT_OPACITY = 0.3f
	}
	...
	override fun onBindViewHolder(holder: ViewHolder, position: Int) {
		...
		binding.nameTextView.alpha = if (item.amount > 0) 1f else CROSSOUT_OPACITY

		binding.addButton.setOnClickListener {
			changeAmountCallback.accept(item, 1)
		}
		binding.subtractButton.alpha = if (item.amount > 0) 1f else CROSSOUT_OPACITY
